...
function renderOrders() {
  ...
  const userOrders = allOrders.filter(
    (order) => order.usuario === session.email
  );
        ...
        <div class="card-body">
          <div class="row">
            ...
            <div class="col-md-4">
              ...
              <p class="mb-1"><strong>Entrega:</strong> ${
                order.cliente.entrega === "delivery"
                  ? "Delivery"
                  : "Retiro en local"
              }</p>
              ${
                order.cliente.direccion
                  ? `<p class="mb-1"><strong>Dirección:</strong> ${order.cliente.direccion}</p>`
                  : ""
              }
              <p class="mb-1"><strong>Pago:</strong> ${order.cliente.pago}</p>
              ${
                order.cliente.fechaDeseada
                  ? `<p class="mb-1"><strong>Fecha deseada:</strong> ${order.cliente.fechaDeseada}</p>`
                  : ""
              }
            </div>
          </div>
